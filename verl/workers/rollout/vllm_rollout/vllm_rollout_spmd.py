...
from verl.workers.rollout.vllm_rollout.utils import (
    ...
    VLLM_LORA_NAME,
    VLLM_LORA_PATH,
    convert_guided_decoding_config,
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        # Convert guided_decoding dict/dataclass to vLLM GuidedDecodingParams.
        # The config loop above picks up guided_decoding as a raw dict or dataclass,
        # but vLLM expects a GuidedDecodingParams object (accesses .backend attribute).
        # Drop it entirely if no actual constraint is set.
        guided_decoding_raw = kwargs.pop("guided_decoding", None)
        converted_gd = convert_guided_decoding_config(guided_decoding_raw)
        if converted_gd is not None:
            kwargs["guided_decoding"] = converted_gd
