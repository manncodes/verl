...
def compute_policy_loss_cispo(
    ...
    """
    Compute the policy loss for CISPO (Clipped Importance Sampling Policy Optimization).

    CISPO clips the importance sampling weights directly, then multiplies by advantages
    and log probabilities. Unlike PPO/GRPO which clip the product of ratio and advantage,
    CISPO preserves gradients for ALL tokens by using detach on the clipped IS weights.

    Key formula:
        L_CISPO = -E[detach(clip(r_t, 1-ε_low, 1+ε_high)) * A_t * log π_θ(a_t|s_t)]

    where r_t = π_θ(a_t|s_t) / π_θ_old(a_t|s_t) = exp(log_prob - old_log_prob)

    This design:
    - Avoids dropping tokens (even rare ones with large updates)
    - Maintains entropy for stable exploration
    - Leverages all tokens for gradient computation

    See MiniMax-M1 paper (https://arxiv.org/abs/2506.13585) for details.
    ...
    Args:
        ...
        loss_agg_mode (str, optional):
            Aggregation mode for `agg_loss`. Defaults to "token-mean".
        config: Actor config containing clip_ratio_low and clip_ratio_high.
        rollout_is_weights: Optional pre-computed IS weights (ignored, computed internally).

    Returns:
        Tuple of (pg_loss, pg_metrics) where pg_metrics contains clipfrac and KL stats.
    """
    assert config is not None
    assert isinstance(config, ActorConfig)
    ...
    # Compute token-level importance ratio: r_t = π_θ / π_θ_old
    negative_approx_kl = log_prob - old_log_prob
    # Clamp for numerical stability
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    ratio = torch.exp(negative_approx_kl)

    # CISPO: Clip the IS weights directly
    ...
    # CISPO loss: -detach(clipped_ratio) * advantage * log_prob
    # The detach ensures gradients flow through log_prob only, preserving ALL token contributions
    pg_losses = -ratio_clipped.detach() * advantages * log_prob
    ...
    pg_metrics = {
        "actor/pg_clipfrac": pg_clipfrac.detach().item(),
        "actor/ppo_kl": ppo_kl.detach().item(),
        "actor/pg_clipfrac_lower": 0.0,  # Not applicable to CISPO
    }
    return pg_loss, pg_metrics
