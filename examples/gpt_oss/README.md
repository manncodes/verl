...
| `USE_TORCH_COMPILE=False` (`actor.use_torch_compile`, `actor.fsdp_config.use_torch_compile`, `ref.use_torch_compile`, `ref.fsdp_config.use_torch_compile`) | off | torch.compile + MoE has been a recurring source of breakage; several fsdp/sglang examples hard-code it off. **verl has two flags per role**: the role-level one (`actor.use_torch_compile`) and the FSDP-engine-level one (`actor.fsdp_config.use_torch_compile`), both default `true`. The latter only wraps `compute_entropy_from_logits` (not the whole module — see `verl/workers/engine/fsdp/transformer_impl.py:147`), but with `dynamic=True` it can still recompile if input shapes change. The launcher sets all four explicitly so neither leaks into a run. |
