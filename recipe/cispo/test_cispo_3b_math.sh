...
#SBATCH --output=/rl/logs/Qwen2.5-3B/cispo/math/vllm_%x_%j.out
#SBATCH --error=/rl/logs/Qwen2.5-3B/cispo/math/vllm_%x_%j.err
...
# ------------------------------------- Setup xp params ---------------------------------------
project_name='RL-CISPO'

adv_estimator=grpo
loss_mode=cispo  # CISPO: Clipped Importance Sampling Policy Optimization
loss_agg_mode="token-mean"  # CISPO works at token level, so token-mean is appropriate
...
# CISPO uses wider clipping bounds than GSPO since it clips IS weights directly
# Recommended starting values based on MiniMax-M1 paper
clip_ratio_low=0.2  # wider clipping than GSPO (0.0003)
clip_ratio_high=0.2  # symmetric clipping bounds
train_batch_size=512
ppo_mini_batch_size=128 # maintain 4 mini-batches
